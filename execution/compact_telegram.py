...
def build_compaction_prompt(chat_history: list, today: str) -> str:
    ...
    lines = [
        "Eres Jeffrey. Resume el siguiente hilo de conversación de Telegram entre Jorge y Jeffrey.",
        "",
    ...
    for msg in chat_history:
        ...
        content = msg["content"]
        # Excluir el trigger de cierre si está al final
...
def _trivial_summary(chat_id: str, chat_history: list, today: str) -> str:
    ...
    msgs = "\n".join(
        f"- [{('Jorge' if m['role'] == 'user' else 'Jeffrey')}]: {m['content']}"
...
def compact(
    ...
    # Paso 1: decidir si llamar al LLM o archivar trivialmente
    if n_messages < MIN_MESSAGES_FOR_LLM:
        summary = _trivial_summary(chat_id, chat_history, today)
    else:
        # Paso 2: generar resumen con Claude
    ...
    # Paso 6: borrar historial — SOLO aquí, tras confirmar escritura
    history[chat_id] = []

    # Paso 7: actualizar índice de memoria
    try:
        update_memory_index(chat_id, path, n_messages)
    except Exception:
        pass  # No crítico: el resumen ya está guardado

    # Paso 8: registrar timestamp de compactación
    history.setdefault("_meta", {}).setdefault(chat_id, {})["last_compaction"] = (
        datetime.now(timezone.utc).isoformat()
    )

    # Paso 9: devolver éxito (save_history lo llama el caller)
