#!/usr/bin/env python3
"""
Puente Telegram <-> Ricky el Project Manager (Claude Code).
...
# Añadir el directorio de Ricky al path para importar compact_telegram local
RICKY_DIR = Path(__file__).parent
sys.path.insert(0, str(RICKY_DIR))
...
PROJECT_DIR = RICKY_DIR.parent.parent  # raíz del proyecto (para .env y binario claude)
HISTORY_FILE = RICKY_DIR / ".tmp" / "telegram_history.json"

load_dotenv(PROJECT_DIR / ".env")

TOKEN = os.getenv("TELEGRAM_TOKEN_RICKY")
...
def build_prompt(chat_history: list, new_message: str) -> str:
    ...
    lines.append("\nResponde como Ricky continuando esta conversación.")
    return "\n".join(lines)
...
def call_claude(prompt: str, claude_bin: str) -> str:
    result = subprocess.run(
        [claude_bin, "-p", prompt, "--add-dir", str(PROJECT_DIR)],
        capture_output=True,
        text=True,
        cwd=str(RICKY_DIR),  # Claude Code carga el CLAUDE.md de Ricky
        timeout=180,
    )
    response = strip_ansi(result.stdout).strip()
    if not response:
        error = result.stderr.strip()
        response = f"[Error al contactar con Ricky: {error[:300]}]" if error else "[Sin respuesta]"
    return response
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Compactación manual ("tema cerrado") ───────────────────────────────────
...
def main() -> None:
    if not TOKEN:
        raise RuntimeError("TELEGRAM_TOKEN_RICKY no está definido en .env")

    app = ApplicationBuilder().token(TOKEN).build()
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    app.add_handler(MessageHandler(filters.VOICE | filters.AUDIO, handle_voice))

    print("Ricky Telegram bridge activo. Esperando mensajes...")
